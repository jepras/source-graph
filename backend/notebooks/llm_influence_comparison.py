...
item_to_research = "Thriller by Michael Jackson"
...
async def test_perplexity_sources():
    """Simple test to check if we can access Perplexity's source metadata."""

    if not PERPLEXITY_API_KEY:
        print("❌ No Perplexity API key found")
        return

    print("🧪 Testing Perplexity source metadata access...")

    # Simple test with Perplexity
    llm = ChatPerplexity(
    ...
    test_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a helpful assistant. Provide a brief answer and cite your sources.",
            ),
            (
                "human",
                "What are the main influences on the video game 'The Last of Us'? Keep it brief.",
            ),
        ]
    )
    ...
    try:
        ...
        if hasattr(response, "response_metadata"):
            print(f"Response metadata keys: {list(response.response_metadata.keys())}")

            # Check for Perplexity-specific fields
            if "citations" in response.response_metadata:
                ...
                print("❌ No 'citations' key in response_metadata")

            if "search_results" in response.response_metadata:
                search_results = response.response_metadata["search_results"]
                print(f"Found {len(search_results)} search results:")
                for i, result in enumerate(search_results, 1):
                    print(
                        f"  {i}. {result.get('title', 'No title')} - {result.get('url', 'No URL')}"
                    )
            else:
                print("❌ No 'search_results' key in response_metadata")

        # Try to access the raw response
        if hasattr(response, "_response"):
            print(f"Raw response type: {type(response._response)}")
            if hasattr(response._response, "citations"):
                print(f"Raw citations: {response._response.citations}")
            if hasattr(response._response, "search_results"):
                print(f"Raw search_results: {response._response.search_results}")

        # Also check if sources are in the content
        if "http" in response.content:
            print("✅ Found URLs in content")
            import re

            urls = re.findall(r'https?://[^\s<>"{}|\\^`\[\]]+', response.content)
            print(f"URLs in content: {urls}")
        else:
...
# Run the test
await test_perplexity_sources()


# =============================================================================
# BLOCK 10: Direct Perplexity API Test
# =============================================================================


async def test_direct_perplexity_api():
    """Test direct Perplexity API call to see raw response structure."""
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant. Provide a brief answer and cite your sources.",
            },
            {
                "role": "user",
                "content": "What are the main influences on the video game 'The Last of Us'? Keep it brief.",
            },
        ],
    }
    ...
    try:
        ...
        if "citations" in result:
            ...
            print("❌ No 'citations' in direct API response")
        ...
        if "search_results" in result:
            ...
            for i, result_item in enumerate(search_results, 1):
                print(
                    f"  {i}. {result_item.get('title', 'No title')} - {result_item.get('url', 'No URL')}"
                )
        else:
            print("❌ No 'search_results' in direct API response")
...
await test_direct_perplexity_api()
